#ifndef __ROO_CONNECT_RABBITMQ_H_
#define __ROO_CONNECT_RABBITMQ_H_
...
public:
    ...
    int initChannel() {
        if (id_ <= 0) {
            log_err("rabbitmq channel invalid id: %d", id_);
            return -1;
        }
        ...
        if( ret_code < 0 ) {
            log_err("rabbitmq amqp_channel_open error with: %d", ret_code);
            return -1;
        }
        is_connected_ = true;
        return 0;
...
#endif  // __ROO_CONNECT_RABBITMQ_H_
