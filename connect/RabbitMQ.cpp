...
int RabbitChannel::amqpErrorCheck(amqp_rpc_reply_t x, const char* context) {
    ...
    switch (x.reply_type) {
        ...
        case AMQP_RESPONSE_NONE:
            log_err("%s missing RPC reply type!\n", context); // conn error
            break;
        ...
        case AMQP_RESPONSE_SERVER_EXCEPTION:
            switch (x.reply.id) {
                case AMQP_CONNECTION_CLOSE_METHOD: {
                    ...
                    log_err("%s: server connection error %uh, message: %.*s\n",
                            context,
                            m->reply_code,
                            (int) m->reply_text.len, (char *) m->reply_text.bytes);

                    log_err("Close connection");
                    closeConnection();
                    break;
                }
                ...
                case AMQP_CHANNEL_CLOSE_METHOD: {
                    ...
                    log_err("%s: server channel error %uh, message: %.*s\n",
                            context,
                            m->reply_code,
                            (int) m->reply_text.len, (char *) m->reply_text.bytes);

                    log_err("Close channel: %d", id_);
                    closeChannel();
                    break;
                }

                default: {
                    log_err("%s: unknown server error, method id 0x%08X\n", context, x.reply.id);
                    closeConnection();
                    break;
                }
            }
...
int RabbitChannel::setConfirmSelect(){
    ...
    if( ret_code < 0 ) {
        log_err("setConfirmSelect return with: %d", ret_code);
        return -1;
    }

    if (res.reply.id != AMQP_CONFIRM_SELECT_OK_METHOD) {
        log_err("expecting AMQP_CONFIRM_SELECT_OK_METHOD, but get reply.id: %d", res.reply.id);
        return -1;
    }
...
int RabbitChannel::declareExchange(const std::string &exchange_name,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_exchange_declare error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::deleteExchange(const std::string &exchange_name,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_exchange_delete error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::bindExchange(const std::string &destination,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_exchange_bind error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::unbindExchange(const std::string &destination,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_exchange_unbind error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::declareQueue(const std::string &queue_name,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_queue_declare error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::deleteQueue(const std::string &queue_name,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_queue_delete error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::bindQueue(const std::string &queue_name,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_queue_bind error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::unbindQueue(const std::string &queue_name,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_queue_unbind error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::purgeQueue(const std::string &queue_name) {
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_queue_purge error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::basicQos(uint16_t message_prefetch_count, bool global_set) {
    ...
    if (qos.prefetch_count != 1) {
        log_err("Attention: qos.prefetch_count = %d", qos.prefetch_count);
    }
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_basic_qos error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::basicRecover(const std::string &consumer) {
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_basic_recover error with: %d", ret_code);
        return -1;
    }
...
int RabbitChannel::basicPublish(const std::string &exchange_name,
    ...
    if (retCode < 0) {
        log_err("amqp_basic_publish fail! ret:%d", retCode);
        goto connection_err;
    }
    ...
    if (AMQP_STATUS_OK != amqp_simple_wait_frame(mqHelper_.connection_, &frame)) {
        log_err("publish ok, but confirm may fail!");
        goto connection_err;
    }
...
int RabbitChannel::basicGet(RabbitMessage& rabbit_msg, const std::string &queue,
    ...
    if (res.reply.id != AMQP_BASIC_GET_OK_METHOD) {
        log_err("unexpeced reply.id: %d", res.reply.id);
        goto error_out1;
    }
    ...
    if(amqp_simple_wait_frame(mqHelper_.connection_, &frame) != AMQP_STATUS_OK){
        log_err("wait for frame header error!");
        goto error_out2;
    }

    if (frame.frame_type != AMQP_FRAME_HEADER){
        log_err("expecting AMQP_FRAME_HEADER, but get: %d", frame.frame_type);
        goto error_out2;
    }
    ...
    if (0 == frame.payload.properties.body_size) {
        ...
        if (!rabbit_msg.envelope.message.body.bytes) {
            log_err("malloc for message body failed!");
            goto error_out3;
        }
    }
    ...
    while (received_size < body_size) {
        ...
        if (frame.frame_type != AMQP_FRAME_BODY) {
            log_err("expecting AMQP_FRAME_BODY, but get: %d", frame.frame_type);
            goto error_out3;
        }
...
int RabbitChannel::basicConsume(const std::string &queue,
    ...
    int ret_code = amqpErrorCheck(res);
    if( ret_code < 0 ) {
        log_err("rabbitmq amqp_basic_consume error with: %d", ret_code);
        return -1;
    }
        ...
        if (AMQP_RESPONSE_LIBRARY_EXCEPTION == ret.reply_type && AMQP_STATUS_TIMEOUT == ret.library_error) {
            ...
            if (AMQP_FRAME_METHOD == frame.frame_type) {
                switch (frame.payload.method.id) {
                    ...
                    default:
                        log_err("An unexpected method was received %u\n", frame.payload.method.id);
                        return -1;
                } // switch
            } // AMQP_FRAME_METHOD
        }
...
bool RabbitMQHelper::doConnect() {
    ...
    for (it = connect_uris_.cbegin(); it != connect_uris_.cend(); ++it){
        ...
        if (amqp_parse_url(uri, &info) != 0) {
            log_err("prase connect_uri failed: %s", uri);
            continue;
        }

        connection_ = amqp_new_connection();
        if (!connection_) {
            log_err("rabbitmq new connect error!");
            continue;
        }
        ...
        if (sock < 0) {
            log_err("rabbitmq socket open error!");
...
bool mq_parse_connect_uris(const std::string& uris, const std::string& passwd, std::vector<std::string>& conn_vec) {
    ...
    if (conn_vec.empty()) {
        log_err("Invalid or empty MQ connect url");
        return false;
    }
...
bool mq_setup_channel_consume_default(RabbitChannelPtr pChannel, void* pArg) {
    ...
    if (!pChannel) {
        log_err("RabbitChannelPtr nullptr!");
        return false;
    }
...
bool mq_setup_channel_publish_default(RabbitChannelPtr pChannel, void* pArg) {
    ...
    if (pChannel->setConfirmSelect() < 0) {
        log_err("Setting publish confirm failed!");
        return false;
    }
